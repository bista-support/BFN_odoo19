...
class CommissionMixin(models.AbstractModel):
    ...
    def default_get(self, fields):
        ...
        agent_ids = self.env.context.get("default_agent_ids")
        if agent_ids:
            res["agent_ids"] = [(6, 0, agent_ids)]
        return res
    ...
    def _compute_agent_ids(self):
        for rec in self:
            if rec.commission_free:
                rec.agent_ids = [(5, 0, 0)]  # Clear agents
                continue

            # Only populate if empty and we got agent ids in context
            if not rec.agent_ids:
                default_agent_ids = rec._context.get('default_agent_ids', [])
                rec.agent_ids = [(0, 0, {'agent_id': agent_id}) for agent_id in default_agent_ids]
    ...
    @api.depends("commission_free", "agent_ids", "product_id.type")
    def _compute_commission_status(self):
        for line in self:
            if line.commission_free:
                ...
                line.commission_status = _("No commission agents")  # Or set to empty: ''
            elif not line.agent_ids:
    ...
    def button_edit_agents(self):
        self.ensure_one()
        self.recompute_agents()  # Make sure agent_ids is up to date
        view = self.env.ref("commission.view_commission_mixin_agent_only")
        return {
            "name": _("Agents"),
            "type": "ir.actions.act_window",
