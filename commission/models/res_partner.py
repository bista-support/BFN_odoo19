...
class ResPartner(models.Model):
    ...
    pm_agent = fields.Boolean(string="SaleManager Agent")
    only_agent = fields.Boolean(compute="_compute_only_agent", store=True)
    

    @api.depends('agent', 'pm_agent')
    def _compute_only_agent(self):
        for rec in self:
            rec.only_agent = rec.agent and not rec.pm_agent
