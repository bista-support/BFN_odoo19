...
class CommissionMakeSettle(models.TransientModel):
    ...
    def _get_agent_lines(self, agent, date_to_agent):
        ...
        all_lines = self.env["account.invoice.line.agent"].search(
            self._get_account_settle_domain(agent, date_to_agent),
            order="invoice_date",
        )
        # Filter out service-type products
        return all_lines.filtered(lambda l: l.object_id.product_id.type != "service")
