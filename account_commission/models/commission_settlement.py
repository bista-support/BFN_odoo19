    ...
    def write(self, vals):
        ...
        for settlement in self:
            if settlement.state == 'invoiced':
                # Get invoice names from settlement lines (Char field)
                invoice_names = settlement.line_ids.mapped('invoice_number')
                if invoice_names:
                    # Fetch matching invoices using name
                    invoices = self.env['account.move'].search([('name', 'in', invoice_names)])
                    if invoices:
                        # ✅ Update Sale Orders
                        ...
                        if sale_orders:
                            sale_orders.write({
                                ...
                                'commission_paid_date': fields.Datetime.now(),
                            })

                        # ✅ Update Invoices
                        invoices.write({
                            'inv_commission_paid': True,
                            'inv_commission_paid_date': fields.Datetime.now(),
                        })
        return res
